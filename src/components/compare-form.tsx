...
export function CompareForm({ initialDevices = [null, null] }: Props) {
  ...
  const [selectedDevices, setSelectedDevices] = useState<(Device | null)[]>(initialDevices);
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {[0, 1].map((index) => (
            <div key={index} className="space-y-4">
              <Select
                value={selectedDevices[index] ? `${selectedDevices[index]?.name}:${selectedDevices[index]?.description}` : ''}
                onValueChange={(value) => {
                  const [name, description] = value.split(':');
                  const device = devices.find(d => d.name === name && d.description === description);
              ...
              >
                ...
                <SelectContent>
                  {devices
                    ...
                    .map(device => (
                      <SelectItem key={`${device.name}:${device.description}`} value={`${device.name}:${device.description}`}>
